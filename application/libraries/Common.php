...
class Common
{
    ...
    function taxlist($id = 0)
    {
        ...
        $tax_list .= '
            <option value="yes" data-tformat="yes">' . $this->PI->lang->line('On') . '</option>
    ...
    function taxlist_edit($id = 0)
    {
        ...
        switch ($id) {
            case 'yes':
                $tax_list .= '<option value="yes"  data-tformat="yes" selected>' . $this->PI->lang->line('On') . '</option>';
                $tax_list .= '<option value="inclusive"  data-tformat="incl" >' . $this->PI->lang->line('Inclusive') . '</option>';

                break;
            case 'incl':
                $tax_list .= '<option value="inclusive"  data-tformat="incl" selected>' . $this->PI->lang->line('Inclusive') . '</option>';
    ...
    function teminat_type($id=0)
    {
        ...
        {
            $this->PI->db->select('*');
            $this->PI->db->from('geopos_teminat_type');
            $this->PI->db->where('id', $id);
            $query = $this->PI->db->get();
            ...
            if($result)
            {
                $id=$result['id'];
                $name=$result['name'];
                $teminat .= '<option selected value="' . $id . '" >' . $name . '</option> ';
            }
            else
                {
                    $teminat .= '<option selected value=0>Seçiniz</option> ';
                }
            ...
            foreach ($result1 as $row) {
                if($row['id']!=$id)
                {
                    $teminat .= '<option value="' . $row['id'] . '" >' . $row['name'] . '</option> ';
                }
    ...
    public function default_warehouse()
    {
        if ($this->PI->aauth->get_user()->loc) {
            ...
            $query = $this->PI->db->get();
            $result = $query->row_array();
            if ($result['ware']) $wr = '<option value="' . $result['key1'] . '">' . $result['title'] . '</option>';
            return $wr;
        }
    }

    public function roles($id=0,$language_id=1)
    {
          
        if ($id!=0) {
            $this->PI->db->select('*');
            $this->PI->db->from('geopos_role');
            $this->PI->db->where('role_id', $id);
            $this->PI->db->where('language_id', $language_id);
            $query = $this->PI->db->get();
            $out = $query->row_array();
        } else {
            $this->PI->db->select('*');
