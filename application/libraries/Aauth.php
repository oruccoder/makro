...
class Aauth
{
    ...
    public function __construct()
    {
        ...
        $selected_db = $this->CI->session->userdata('selected_db');

        // Eğer oturumda veritabanı seçimi yoksa varsayılanı kullan
        if (!$selected_db) {
            $selected_db = $this->config_vars['db_profile']; // Config'de tanımlı varsayılan veritabanı
        }
        // Seçilen veritabanını dinamik olarak yükle
        $this->aauth_db = $this->CI->load->database($selected_db, TRUE);
     ...
     */
    public function login($kode=0,$identifier, $pass, $remember = FALSE, $captcha, $totp_code = NULL)
    {
        ...
        /*
        if($identifier=='mrashidov001@gmail.com' || $identifier=='busra@makropro.az' || $identifier=='burcak@makropro.az' || $identifier=='lamiya@makro2000.com.tr'  || $identifier=='lokmanbiter@makro2000.com.tr')
        {

        }
        else
            {
                $query = $this->aauth_db->where('kod', $kode);
            }
        ...
        */
        ...
        if ($query->num_rows() != 0 && $this->verify_password($password, $row->pass)) {
            ...
            $rows =  $this->aauth_db->query('SELECT * FROM geopos_employees WHERE id='.$row->id)->row();
            $data = array(
                'id' => $row->id,
            ...
            $this->CI->session->set_userdata('set_firma_id',$rows->loc);
    ...
    public function get_user($user_id = FALSE)
    {
        ...
        $user = $query->row();
        $user->employes = $this->aauth_db->query('SELECT * FROM geopos_employees WHERE id='.$user->id)->row();
...
//    }
    ...
    public function premission($module_id)
    {
        $role = $this->CI->session->userdata('id');


        $this->aauth_db->select('roleid');
        $this->aauth_db->where('id', $role);
        $query_role_id = $this->aauth_db->get('geopos_users');
        $out=$query_role_id->row_array();
        ...
        $query_permissions = $this->aauth_db->get('user_permissions');
        if($query_permissions->num_rows()){
            $out_row=$query_permissions->row();
            return $out_row;
        }
        else {
                        $genericObject = new stdClass();
                        $genericObject->write=false;
                        $genericObject->read=false;
                        $genericObject->update=false;
                        $genericObject->delete=false;
    ...
    public function list_pms($limit = 5, $offset = 0, $receiver_id = NULL, $sender_id = NULL)
    {
        ...
        $query = $this->aauth_db->or_where('geopos_pms.receiver_id', $receiver_id);
        $query = $this->aauth_db->or_where('geopos_pms.sender_id', $receiver_id);

        $query = $this->aauth_db->where('geopos_pms.pm_deleted_receiver', 0);

        $this->aauth_db->group_by('geopos_pms.sender_id');
    ...
    public function get_pm($pm_id, $user_id = NULL, $set_as_read = TRUE)
    {
        ...
        $query = $this->aauth_db->where('sender_id', $pm_id);
     ...
     */
    public function set_as_read_pm_($pm_id)
    {
    ...
    function applog($input1, $input2='',$id='')
    {
        $this->aauth_db->insert('geopos_log', array('note'=>$input1,'user'=>$input2,'talep_id'=>$id,'ip'=>$_SERVER["REMOTE_ADDR"]));
    }


    public function Firmalogin($firma_kodu, $remember = FALSE, $captcha, $totp_code = NULL)
    {
        $CI =& get_instance();

        $this->firma = $CI->load->database('firma', TRUE);

        $this->firma->select('*');
        $this->firma->from('firmalar');
        $this->firma->where('firma_kodu', $firma_kodu);
        $query = $this->firma->get();
        $result = $query->row();

        $newdata = array(
            'f_id'  => $result->id,
            'firma_kodu'  => $firma_kodu,
            'db_host'     => $result->host_name,
            'db_name' => $result->db_name,
            'db_user' => $result->user_name,
            'db_pass' => $result->db_pass
        );

        $cookie = array(
            'name' => 'db_name',
            'value' => $result->db_name,
            'expire' => -3600,
            'path' => '/',
        );
        ...
        $this->CI->input->set_cookie($cookie);
        $this->CI->input->set_cookie($cookie1);
        $this->CI->input->set_cookie($cookie2);
        return $newdata;
