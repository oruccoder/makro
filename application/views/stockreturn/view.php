                    ...
                    <div class="row wrapper white-bg page-heading">
                        ...
                        <div class="col-lg-12">
                            ...
                            if ($invoice['status'] != 'canceled') { ?>
                                ...
                                {
                                    case 0 : $eurl='edit?id=' . $invoice['iid']; $type_i=$this->lang->line('Stock Return');$purl= 'printinvoice?id=' . $invoice['iid'].'&ty=0';
                                    $curl=base_url('supplier/view?id=' . $invoice['cid']);
                                    break;
                                    case 1 : $eurl='edit_c?id=' . $invoice['iid']; $type_i=$this->lang->line('Stock Return');$purl= 'printinvoice?id=' . $invoice['iid'].'&ty=1';
                                    $curl=base_url('customers/view?id=' . $invoice['cid']);
                                    break;
                                    case 2 : $eurl='edit_note?id=' . $invoice['iid']; $type_i=$this->lang->line('Credit Note');$purl= 'printinvoice?id=' . $invoice['iid'].'&ty=2';
                                     $curl=base_url('customers/view?id=' . $invoice['cid']);
                                    break;
                                }
                    ...
                    <div class="row">
                        <table class="table table-striped">
                            ...
                            <?php foreach ($attach as $row) {
                                ...
                                echo '<tr><td><a data-url="' . base_url() . 'purchase/file_handling?op=delete&name=' . $row['col1'] . '&invoice=' . $invoice['iid'] . '" class="aj_delete"><i class="btn-danger btn-lg icon-trash-a"></i></a> <a class="n_item" href="' . base_url() . 'userfiles/attach/' . $row['col1'] . '"> ' . $row['col1'] . ' </a></td></tr>';
                            } ?>
            ...
            dataType: 'json',
             formData: {'<?=$this->security->get_csrf_token_name()?>': crsf_hash},
                ...
                $.each(data.result.files, function (index, file) {
                    $('#files').append('<tr><td><a data-url="<?php echo base_url() ?>purchase/file_handling?op=delete&name=' + file.name + '&invoice=<?php echo $invoice['iid'] ?>" class="aj_delete"><i class="btn-danger btn-sm icon-trash-a"></i> ' + file.name + ' </a></td></tr>');
