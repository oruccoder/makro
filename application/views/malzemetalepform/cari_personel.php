                        ...
                        <div class="col-12">
                            ...
                            <div class="table-responsive">
                                ...
                                <span data-popup="popover" title="Proje Adı" data-trigger="hover" data-content="<?php echo proje_name($details->proje_id)?>">
                                    ...
                                    <tr>
                                        <th style="text-align: center" colspan="2"><button type="button" class="btn btn-info cari_atama"><i class="fa fa-users"></i> Seçili Ürünlere Cari Ata</button></th>
                                        <th style="text-align: center" colspan="2"><button type="button" class="btn btn-info teklif_asama_update"><i class="fa fa-users"></i> Seçili Ürünlere Teklif Aşamasına Al</button></th>
                                    </tr>
                                    </thead>
                                </table>
                            </div>
                    ...
                    <div class="col-12">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered malzemetalep_products">
                                <thead>
                                <tr>
                                    <td><input type="checkbox" class="form-control all_checked"></td>
                                    <td>No</td>
                                    <td>Resim</td>
                                    <td>Malzeme</td>
                                    <td>Tanım</td>
                                    ...
                                    <td>Durum</td>
                                    <td>Teklif Verilecek Cariler</td>
                                ...
                                <?php if($talep_products): ?>
                                    <?php foreach ($talep_products as $key => $items): ?>
                                        <tr>
                                            <td><input type="checkbox" class="form-control one_checked" data-item-id="<?=$items['id']?>"></td> <!-- Artırılmış key değeri -->
                                            ...
                                            <td><?php echo (malzemetalep_item_cari_kontrol($items['id'])['status']) ? malzemetalep_item_cari_kontrol($items['id'])['html'] : 'Cari Atanmamış' ; ?></td>
...
<script>
    ...
    $(document).on('click','.cari_atama',function() {
        ...
        <div>
            <p>Cari Seçiniz</p>
            <select class="form-control select-box teklif_cari_id select2-personel" multiple>
                ${CustomerOptions()}
            </select>
        </div>
    `,
            buttons: {
                formSubmit: {
                    ...
                    action: function () {
                        ...
                        const teklif_cari_id = $('.teklif_cari_id').val();
                        if (!teklif_cari_id || teklif_cari_id === '0') {
                            $.alert({
                                theme: 'material',
                                icon: 'fa fa-exclamation',
                                type: 'red',
                                ...
                                columnClass: "col-md-4 mx-auto",
                                title: 'Dikkat!',
                                content: 'Lütfen bir Cari seçiniz!',
                                buttons: {
                                    prev: {
                                        text: 'Tamam',
                                        btnClass: "btn btn-link text-dark"
                        ...
                        $.ajax({
                            url: baseurl + 'malzemetalepform/item_cari_atama',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                teklif_cari_id: teklif_cari_id,
    ...
    $(document).on('click','.teklif_asama_update',function() {
        let checkedCount = $('.one_checked:checked').length;
        ...
        $.confirm({
            ...
            smoothContent: true,
            draggable: false,
            content: 'Teklif Aşamasına Alınacaktır Emin Misiniz',
            buttons: {
                formSubmit: {
                    text: 'Geçiş Yap',
                    btnClass: 'btn btn-info btn-sm',
                    action: function () {
                        // AJAX ile sunucuya veri gönder
                        $.ajax({
                            url: baseurl + 'malzemetalepform/teklif_asama_update',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                urunDetails: urunDetails,
