...
<head>
    ...
    <style>
            ...
            html,body{margin:0;padding:0;font-size: 9pt;background-color: #fff;}
 #products {
               width:100%;
            }
            #products tr td{
...
<body>
<h3 id="logo" class="text-center"><br><img style="max-height:100px;" src='<?php  $loc=location($invoice['loc']);  echo base_url('userfiles/company/' . $loc['logo']);
...
$this->pheight=$this->pheight+12;
   ...
   <div class="text-center">  <?php echo $this->lang->line('Thank you') ?></div>
    <?php if($qrc AND $invoice['status'] !='paid') { ?>
        <div class="text-center"><small><?php echo $this->lang->line('Scan & Pay') ?></small><img style="max-height:230px;" src='<?php echo base_url('userfiles/pos_temp/' . $qrc) ?>' alt='QR'></div>
    <?php } else {
