                                ...
                                <div class="title-action">
                                    ...
                                    <button type="button" class="btn btn-success mb-1 btn-min-width dropdown-toggle"
                                            data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i
                                                class="icon-print"></i> <?php echo $this->lang->line('POS Print') ?>
                                    </button>
                                    <div class="dropdown-menu">
                                      ...
                                      <?php if($printer['val2']=='server') { echo'<a class="dropdown-item" id="thermal_server"
                                           data-ptid="'.$invoice['iid'].'"  data-url="'.$printer['val3'].'">'.$this->lang->line('Thermal Print') .'</a>';
                                      }
                                      else { echo'<a class="dropdown-item" id="thermal_p"
                                           data-ptid="'.$invoice['iid'].'"  data-url="'.$printer['val3'].'">'.$this->lang->line('Thermal Print') .'</a>'; }
                                           ?>
                            ...
                            <ul class="px-0 list-unstyled">
                                ...
                                <li class="text-bold-800"><a
                                            href="<?php echo base_url('customers/view?id=' . $invoice['cid']) ?>"><strong
                                                class="invoice_a"><?php echo $invoice['name'] . '</strong></a></li><li>' . $invoice['company'] . '</li><li>' . $invoice['address'] . '</li><li>' . $invoice['city'] . ',' . $invoice['country'] . '</li><li>'.$this->lang->line('Phone') .': ' . $invoice['phone'] . '</li><li>'.$this->lang->line('Email') .': ' . $invoice['email']; ?>
                                </li>
                            </ul>
                    ...
                    <div class="row">
                        <table class="table table-striped">
                            ...
                            <?php foreach ($attach as $row) {
                                ...
                                echo '<tr><td><a data-url="' . base_url() . 'invoices/file_handling?op=delete&name=' . $row['col1'] . '&invoice=' . $invoice['iid'] . '" class="aj_delete"><i class="btn-danger btn-lg icon-trash-a"></i></a> <a class="n_item" href="' . base_url() . 'userfiles/attach/' . $row['col1'] . '"> ' . $row['col1'] . ' </a></td></tr>';
                            } ?>
            ...
            dataType: 'json',
             formData: {'<?=$this->security->get_csrf_token_name()?>': crsf_hash},
                ...
                $.each(data.result.files, function (index, file) {
                    $('#files').append('<tr><td><a data-url="<?php echo base_url() ?>invoices/file_handling?op=delete&name=' + file.name + '&invoice=<?php echo $invoice['iid'] ?>" class="aj_delete"><i class="btn-danger btn-sm icon-trash-a"></i> ' + file.name + ' </a></td></tr>');
