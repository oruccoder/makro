        ...
        <div class="card-content">
            ...
            <div class="content-body">
                ...
                if($invoice['talep_or_invoice']==1) {
                    $details = invoice_details($inv);
                    $dec = $details->invoice_type_desc . ' ' . $details->invoice_no;
                    if ($details->invoice_type_id == 29 || $details->invoice_type_id == 30) {
                        $href = "/billing/print_form?id=" . $inv . "&token=$token";
                    } else {
                        $href = "/billing/printinvoice_?id=" . $inv . "&token=$token";
                    }

                } else {
                        $details = talep_details($invoice['invoice_id']);
                        $dec=$details->talep_no;
                        $href = "/billing/print_satinalma?id=".$inv;
                    }
                    $invoice_bilgileri="<a href='$href' class='btn btn-info btn-sm'>$dec Görüntüle</a>";
                    //$invoice_bilgileri="<a href='/billing/printinvoice_?id=$inv&token=$token' class='btn btn-info'>Faturayı Önizle</a>";
...
?>
                    ...
                    <div id="invoice-company-details" class="row mt-2">
                        <div class="col-md-6 col-sm-12 text-xs-center text-md-left">
                            ...
                            <p class="font_11"> <?php echo 'Talep Eden : ' . personel_details($invoice['kullanici_id']) . '</p>';?>
                            <p class="font_11">Avans Talep Formu</p>
                                        ...
                                        foreach ($products as $row) {
                                            ...
                                            if(onay_durumlari_ogren_product(4,$_GET['id'],$product_id))
                                            {
                                                ...
                                                echo '<tr>
                                                    ...
                                                    <td>' .$pers_cari_name.' - '. $row['product_name'] .' '.$row['product_detail']. '</td>
...
<script>
    ...
    $(document).on('click',"#tumunu_onayla",function(e) {
        ...
        jQuery.ajax({
            url: baseurl + 'billing/avans_talep_product_status_toplu',
            type: 'POST',
