...
<head>
    ...
    <title>Avans Talep Formu #<?php echo $invoice['id'] ?></title>
    <style>
    ...
    <table class="plist" cellpadding="0" cellspacing="0">
        ...
        foreach ($products as $row) {
            ...
            else
            {
                ...
                $pers_cari_name=customer_details($invoice['talep_eden_pers_id'])['name'];
            }
            $sub_t += $row['subtotal'];
            echo '<tr>
                    <td width="20%">' . $c . '</td>
                   <td width="20%">' .$row['product_name'] . '</td>
                     <td width="20%">' . floatval($row['qty']) .' '. units_($row['unit'])['name'] . '</td>
                     <td width="20%">' . amountFormat($row['price']) . '</td>
                    <td width="20%">' .amountFormat($row['subtotal']). '</td>
                </tr>';
