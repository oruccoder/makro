        ...
        <div class="card-content">
            ...
            <div class="card-body">
                <form method="post" id="data_form">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-2 invoice">
                                    <label for="taxformat" class="col-form-label">Tip Seçinizi<spanr style="color: red">*</spanr></label>
                                    <select class="form-control" name="talep_or_invoice" id="talep_or_invoice">
                                        <option value="1">Fatura / Forma2</option>
                                        <option value="2">SatınAlma Talep</option>
                                        <option value="3">Lojistik SF</option>
                                    </select>
                                </div>

                                <div class="col-sm-2">
                                    <label for="taxformat" class="col-form-label"><?php echo $this->lang->line('talep_eden_muesese') ?><spanr style="color: red">*</spanr></label>
                                    <select class="form-control select-box required" name="talep_eden_pers_id"  id="talep_eden_pers_id">
                                ...
                                <div class="col-sm-2 invoice">
                                    <label for="taxformat" class="col-form-label">Formlar</label>
                                ...
                                <div class="col-md-2">
                                    <label class="col-form-label"
                                           for="birthday"><?php echo $this->lang->line('Description') ?><span style="color: red">*</span></label>

                                    <div class="input-group">
                                        <input type="text" placeholder="Avans Talebi Hakkında Malumat Veriniz"
                                               class="form-control margin-bottom required"  name="description" autocomplete="false">
                                    </div>
                                </div>
                                <div class="col-sm-2">
                    ...
                    <div id="saman-row" >
                        <table class="table-responsive tfr my_stripe">
                            ...
                            <tr>
                                ...
                                <td><input type="text" class="form-control product_name required" name="product_name[]" id="product_name-0"
                                ...
                                <td><input type="text" class="form-control req prc required" name="product_price[]" id="price-0"
                                           onkeypress="return isNumber(event)" onkeyup="rowTotal_gider('0'), billUpyog_gider()"
                                           autocomplete="off"></td>
                    ...
                    <input type="hidden" value="form/action_avans" id="action-url">
...
<script type="text/javascript">
    ...
    $('#addproduct_talep').on('click', function () {
        ...
        data = '<tr>' +
            '<td><input type="text" class="form-control product_name required" id="product_name-'+cvalue+'" name="product_name[]" ' +
        ...
        $('.prc').keyup(function (){
            let total = $('option:selected', '#invoice').attr('form_total');
            let totals=0;
            let count = $('.prc').length;
            for(let i = 0; i < count; i++){
                totals += parseFloat($('.prc').eq(i).val())
            }
            let val =  $(this).val();
            if(parseFloat(totals) > parseFloat(total)){
                $.alert({
                    theme: 'material',
                    ...
                    title: 'Dikkat!',
                    content: 'Tutar Seçtiğiniz Form Tutarından Büyük Olamaz!',
                    buttons:{
                        prev: {
                            text: 'Tamam',
    ...
    $(document).on('change', "#talep_eden_pers_id", function (e) {
        ...
        var cari_pers = $(this).val();
        var talep_or_invoice = $('#talep_or_invoice').val();
        $.ajax({
            type: "POST",
            url: baseurl + 'form/cari_pers_list_invoice',
            data:'talep_or_invoice='+talep_or_invoice+'&cari_id='+cari_pers+'&'+'&'+crsf_token+'='+crsf_hash,
            success: function (data) {
                if(data)
                {
                    ...
                    jQuery.each(jQuery.parseJSON(data), function (key, item) {
                        $("#invoice").append('<option status="'+item.status+'" beklyen_total="'+item.beklyen_total+'" form_total="'+item.total+'" value="'+ item.id +'">'+ item.name +'</option>');
                    });
                    //$('#pay_type').append($('<option>').val(3).text('Tahsilat'));
                }
    ...
    $('.prc').keyup(function (){
        let total = $('option:selected', '#invoice').attr('form_total');
        let status = $('option:selected', '#invoice').attr('status');
        let beklyen_total = $('option:selected', '#invoice').attr('beklyen_total');
        let totals=0;
        let count = $('.prc').length;
        for(let i = 0; i < count; i++){
            totals += parseFloat($('.prc').eq(i).val())
        ...
        if(beklyen_total > 0){
            let kalan = parseFloat(total)-parseFloat(beklyen_total);
            ...
            if(kalan < totals){
                $.alert({
                    ...
                    columnClass: "col-md-4 mx-auto",
                    title: 'Dikkat!',
                    content: 'Kalan Bakiyeden Fazla Giremezsiniz.Talep Edilebilecek Tutar :  '+kalan,
                    buttons:{
                        prev: {
                            text: 'Tamam',
                            btnClass: "btn btn-link text-dark",
