<div class="page-header">
    <div class="page-header-content header-elements-lg-inline">
        <div class="page-title d-flex">
            <h4><span class="font-weight-semibold">Yeni Forma 2</span></h4>
                    ...
                    <div class="card-body">
                        <form method="post" id="data_form">
                            <fieldset class="mb-3">
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-2">
                                        ...
                                        <select class="select-box form-control" id="customer_id" name="customer_id">
                                            <option value="0">Seçiniz</option>
                                            <?php foreach (all_customer() as $customers)
                                            {
                                                echo "<option name='$customers->company' value='$customers->id'>$customers->company</option>";
                                            } ?>
                                        </select>
                                    </div>
                                    <?php
                                    ...
                                    <?php } ?>
                                    <div class="col-lg-<?php echo $sm ?>">
                                        <label for="fis_no" class="caption">Protokol No</label>
                                        <select class="form-control select-box required" id="hizmet_protokol" name="hizmet_protokol">
                                ...
                                <div class="form-group row">
                                    <div class="col-lg-2">
                                        <label for="fis_note" class="caption"><?php echo $this->lang->line('forma2_note') ?></label>
                                        <textarea class="form-control " name="fis_note"  rows="1"><?php echo isset($edit_data['notes'])?$edit_data['notes']:'' ?></textarea>
                                    </div>
                                    <div class="col-lg-2">
                                        <label for="fis_note"  class="caption">Muqavele No</label>
                                        <input class="form-control required" name="muqavele_no_new">
                                    </div>
                                    <div class="col-lg-2">
                                        <label for="kdv_durum"  class="caption">KDV Durumu</label>
                                        <select class="form-control tax_status"  name="kdv_durum">
                                    ...
                                    <div class="col-lg-2">
                                        <label for="kdv_oran"  class="caption">KDV Oran</label>
                                        <input class="form-control kdv_oran"  name="kdv_oran">
                                    </div>
                                </div>
                            </fieldset>
                            <fieldset class="mb-2">
                                <table class="my_stripe table datatable-show-all" cellspacing="0" width="100%">
                                ...
                                <table>
                                    <tr class="sub_c" style="display: table-row;">
                                        ...
                                        <td align="right" colspan="9"><input type="button" class="btn btn-success sub-btn btn-lg"
                            ...
                            if(isset($id)){
                                ...
                                <input type="hidden" value="formainvoices/save?id=<?php echo $id ?>" id="action-url">
                            <?php } else { ?>
                                <input type="hidden" value="formainvoices/save" id="action-url">
...
<script>
    ...
    $(document).on('change','#customer_id',function (){
        ...
        $.post(baseurl + 'customers/customer_pay_list',data,(response) => {
            ...
            if(responses.status==200){
                if(responses.cari_protokol.length > 0){
                    ...
                    responses.cari_protokol.forEach((item_,index) => {
                        ...
                        $('#hizmet_protokol').append('<option value="'+item_.id+'">'+item_.code+'-'+item_.muqavele_no+'</option>');;
                    })
    ...
    $(document).on('change','#hizmet_protokol',function (e) {
        ...
        $.post(baseurl + 'razilastirma/get_item',data_post,(response) => {
            ...
            if(responses.details.length > 0){
                ...
                $('.kdv_oran').val(responses.details_new.tax_oran);
                $('.tax_status').val(responses.details_new.tax_status);
                $('.my_stripe tbody tr').remove();
                let i = 0;
                responses.details.forEach((items,j) => {
                    items.forEach((item_, index) => {
                        ...
                        $.post(baseurl + 'razilastirma/get_item_details', data_post_v, (response_item) => {
                            ...
                            var data = '<tr>' +
                                ...
                                '</td> ' +
                                '<td><input type="number" class="form-control req unt qiymet_hesap" indexval="'+cvalue+'"  maxlength="' + birim_fiyati + '" name="product_qiymet[]" value="' + birim_fiyati + '"  onkeyup="rowTotal_form(' + cvalue + ')" id="qiymet-' + cvalue + '"></td>' +
                                ...
                                '</tr>';
                            //ajax request
                            $('.my_stripe tbody').append(data);
                            // $('tr.last-item-row').append(data);
                            i++;
    ...
    $(document).on('click','#submit-data-forma2',function (){
        $.confirm({
            theme: 'modern',
            closeIcon: true,
            title: 'Forma 2 Oluşturma ',
            icon: 'fa fa-plus',
            type: 'dark',
            animation: 'scale',
            useBootstrap: true,
            ...
            smoothContent: true,
            draggable: false,
            content:'Forma 2 Oluşturulacak Emin Misiniz?',
            buttons: {
                formSubmit: {
                    text: 'Evet',
                    btnClass: 'btn-blue',
                    action: function () {
                        ...
                        $.post(baseurl + action_url,$('#data_form').serialize(),(response)=>{
    ...
    $(document).on('keyup', '.qiymet_hesap', function (e) {
        ...
        // let max = document.getElementById("qiymet-"+eq).attributes.maxlength.value
        //
    ...
    $(document).on('keyup', '.asama', function (e) {
        let eq = $(this).attr("eq");
        var bolum_id = formInputGet("#bolum_id_val-", eq);
    ...
    $(document).on('click', '.clone', function (e) {
        ...
        setTimeout(function(){
            ...
            var data = '<tr>' +
                ...
                '<input type="numaric" class="form-control amnt qiymet_hesap" indexval="'+cvalue+'" value="'+quantity+'" name="product_qty[]"  id="product_qty-' + cvalue + '" >' +
                '</td> ' +
                '<td><input type="number" class="form-control req unt qiymet_hesap" indexval="'+cvalue+'"  name="product_qiymet[]"  maxlength="'+birim_fiyati+'" value="'+birim_fiyati+'"  onkeyup="rowTotal_form(' + cvalue + ')" id="qiymet-' + cvalue + '"></td>'+
                '<td><input type="text" class="form-control req unt" disabled  value="'+total_fiyat+'" id="cemi-' + cvalue + '"></td>'+
                '<td class="text-center"><button type="button" data-rowid="' + cvalue + '" class="btn btn-danger btn-sm removeProd" title="Remove" >' +
