<div class="page-header">
    <div class="page-header-content header-elements-lg-inline">
        <div class="page-title d-flex">
            <h4><span class="font-weight-semibold">Qaime Düzenle</span></h4>
                    ...
                    <div class="col-md-6"> <!-- Cari Bilgileri -->
                        <div class="form-group row">
                            <div class="frmSearch col-sm-6">
                                ...
                                <select class="select-box form-control required" name="customer_id" id="customer_id">
                                    ...
                                    <?php foreach (all_customer() as $customer_item): ?>
                                        <option value="<?= $customer_item->id; ?>" <?= $invoice->csd == $customer_item->id ? 'selected' : ''; ?>>
                                            <?= $customer_item->company; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="frmSearch col-sm-6">
                                ...
                                <select class="form-control select-box required" name="proje_id" id="proje_id">
                                    ...
                                    <?php foreach (all_projects() as $project): ?>
                                        <option value="<?= $project->id; ?>" <?= $invoice->proje_id == $project->id ? 'selected' : ''; ?>>
                                            <?= $project->name; ?>
                                        </option>
                                    <?php endforeach; ?>
                            ...
                            <div class="frmSearch col-sm-12">
                                ...
                                <textarea class="form-control required" name="notes" rows="2" id="notes"><?=$invoice->notes?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6"  style="border-left: 1px solid gray"> <!-- Fatura Bilgileri -->
                        <div class="form-group row">
                            <div class="frmSearch col-sm-3">
                                ...
                                <input type="text" class="zorunlu form-control required" placeholder="Invoice #" name="invoice_no" value="<?=$invoice->invoice_no?>" id="invoice_no">
                            </div>
                            <div class="frmSearch col-sm-3">
                                <label for="invocieno" class="caption">Referans</label>
                                <input type="text" class="form-control  " placeholder="Reference #" name="refer" id="refer" value="<?=$invoice->refer?>">
                            </div>
                            <div class="frmSearch col-sm-3">
                                <label for="invocieno" class="caption">Fatura Tarihi</label><span class='text-danger'>*</span>
                                <?php $formattedDate = DateTime::createFromFormat('Y-m-d H:i:s', $invoice->invoicedate)->format('Y-m-d');
                                ?>
                                <input type="date" class="form-control  required" placeholder="Fatura Tarihi" name="invoicedate" id="invoice_date" value="<?=$formattedDate?>">
                            </div>
                            <div class="frmSearch col-sm-3">
                                <label for="invocieno" class="caption">Ödeme Vade Tarihi</label>
                                <?php $formattedDate2 = DateTime::createFromFormat('Y-m-d H:i:s', $invoice->invoiceduedate)->format('Y-m-d');
                                ?>
                                <input type="date" class="form-control " placeholder="Fatura Tarihi" name="invocieduedate" id="invocieduedate" value="<?=$formattedDate2?>">
                            </div>
                            <div class="frmSearch col-sm-3">
                                ...
                                <select name="alt_cari_id" id="alt_cari_id" class="form-control select-box">
                                    <?php foreach (all_customer() as $customer_item): ?>
                                        <option value="0">Seçiniz</option>
                                        <option value="<?= $customer_item->id; ?>" <?= $invoice->alt_cari_id == $customer_item->id ? 'selected' : ''; ?>>
                                            <?= $customer_item->company; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="frmSearch col-sm-3">
                                ...
                                <select name="para_birimi" id="para_birimi" class="form-control">
                                    ...
                                    <?php foreach (para_birimi() as $row): ?>
                                        <option value="0">Seçiniz</option>
                                        <option value="<?= $row['id']; ?>" <?= $invoice->para_birimi == $row['id'] ? 'selected' : ''; ?>>
                                            <?= $row['code']; ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="frmSearch col-sm-2">
                                ...
                                <input type="text" class="form-control required" placeholder="Kur" name="kur_degeri" id="kur_degeri" value="<?= $invoice->kur_degeri?>">
                            </div>

                            <div class="frmSearch col-sm-2">
                                <label for="invocieno" class="caption">KDV</label>
                                <select class="form-control" id="taxformat">
                                    <option value="0" <?= $invoice->taxstatus == 'no' ? 'selected' : ''; ?>>Hariç</option>
                                    <option value="1" <?= $invoice->taxstatus == 'yes' ? 'selected' : ''; ?>>Dahil</option>
                                </select>
                            </div>
                            <div class="frmSearch col-sm-2">
                                <label for="invocieno" class="caption">KDV Oran</label>
                                <input type="text" class="form-control required"  name="kdv_oran" id="kdv_oran" value="<?= $invoice->tax_oran?>">

                            </div>
                            <div class="frmSearch col-sm-6">
                                <label for="invocieno" class="caption">İskonto Tipi</label>
                                <select class="form-control " id="discount_format" name="discount_format">
                                        <option  value='%' <?= $invoice->format_discount == '%' ? 'selected' : ''; ?>>Yüzde (%)</option>
                                        <option  <?= $invoice->format_discount == 'flat' ? 'selected' : ''; ?> value='flat'>Sabit</option>";
                                </select>
                            </div>
                            <div class="frmSearch col-sm-6">
                                <label for="invocieno" class="caption">İskonto</label>
                                <input type="number" class="form-control discount_rate" value="<?= $invoice->discount_rate?>" placeholder="İndirim"  name="discount_rate" id="discount_rate"
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered" id="product_table">
                        ...
                        <?php foreach ($products as $key =>$item) {
                            ...
                            if($invoice->invoice_type_id==24 || $invoice->invoice_type_id==41){
                                $xx= who_demirbas($item->demirbas_id)->name;
                            }
                            else {
                                $xx= $item->product_code;
                            }
                            ...
                            <tr>
                                <th>1</th>
                                <th><select class="form-control select-box product_name" style="width: 550px">
                                        <option selected value="<?=$item->pid?>"><?=$product_name?></option>
                                    </select>
                                    <input type='hidden' class='product_stock_code_id_input' value="<?=$item->product_stock_code_id?>">
                                    <input type='hidden' class='product_demirbas_id' value="<?=$item->demirbas_id?>">
                                </th>
                                <th><input class="form-control product_price" type="number" value="<?=$item->price?>"></th>
                                <th><input class="form-control product_qty" value="<?=$item->qty?>" min="1" type="number"></th>
                                <th><select class="select2 form-control product_unit_id select-box" style="max-width: 250px">
                                        <?php foreach (units() as $unit_item) { ?>
                                            <option
                                                    value="<?= $unit_item['id'] ?>"
                                                <?= $item->unit == $unit_item['id'] ? 'selected' : '' ?>>
                                                <?= $unit_item['name'] ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                                </th>
                                <th><input class="form-control edv_oran" value="<?=$item->tax?>" type="number"></th>
                                <th><input class="form-control indirim_oran" value="<?=$item->discount?>"  min="0" type="number"></th>
                                <th><input class="form-control cemi_edvsiz" value="<?=$item->subtotal?>" disabled type="number"></th>
                                <th>
                                 <?php if($key == 0)  {
                                     echo '<button class="btn btn-success new_add_line"><i class="fa fa-plus"></i></button>';

                                 }
                                 else {
                                     echo '<button class="btn btn-danger btn-sm remove_row"><i class="fa fa-trash"></i></button>';
                                 }?>
                                </th>
                            </tr>
                            <?php
                        ...
                        <tr>
                            <th colspan="8" class="text-center"><button type="button" class="btn btn-info" id="create_form"><i class="fa fa-save"></i> Qaime Güncelle</button></th>
                        </tr>
...
<script>
    $(document).ready(function () {
        ...
        $('.product_name').select2({
            ...
            ajax: {
                url: "<?php echo base_url('malzemetalepform/search_products'); ?>",
                dataType: 'json',
                method: 'POST',
                delay: 250,
        ...
        //edit bölümü
        // PHP'den gelen bolum_id ve asama_id
        const selectedBolumId = "<?= $invoice->bolum_id ?>";
        const selectedAsamaId = "<?= $invoice->asama_id ?>";
        // Proje değiştiğinde bölümleri ve depoyu yükle
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery.each(data, function (key, item) {
                        $("#bolum_id").append(
                            '<option value="' + item.id + '"' + (item.id == selectedBolumId ? ' selected' : '') + '>' + item.name + '</option>'
                        );
                    });

                    // Bölüm varsa aşamaları tetikle
                    if (selectedBolumId) {
                        $('#bolum_id').trigger('change');
                    }
                }
            });
        ...
        // Bölüm değiştiğinde aşamaları yükle
        $(document).on('change', '#bolum_id', function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    jQuery.each(data, function (key, item) {
                        $("#asama_id").append(
                            '<option value="' + item.id + '"' + (item.id == selectedAsamaId ? ' selected' : '') + '>' + item.name + '</option>'
                        );
                    });
                }
            });
        });
        // Sayfa yüklendiğinde proje seçimini tetikle
        $('#proje_id').trigger('change');
        //edit bölümü
        ...
        <tr>
            <td>${rowCount}</td>
                            ...
                            <input type='hidden' class='product_stock_code_id_input' value='0'>
                            <input type='hidden' class='product_demirbas_id' value='0'>
                            </th>
    ...
    `;
        ...
        $('#product_table').on('change', '.product_name', function () {
            ...
            console.log("Seçilen Data:", selectedData); // Kontrol amaçlı log
            ...
            if (selectedData && selectedData.demirbas_id) {
                let productdemirbasid = selectedData.demirbas_id;
                // Değeri başka bir input alanına yazdırma
                $(this).closest('tr').find('.product_demirbas_id').val(productdemirbasid);
            } else {
                console.log("Product Stock Code ID bulunamadı!");
            }
        });
        ...
        const selectedValues = <?= json_encode($search_input_nakliye); ?>; // PHP'den gelen array
        const selectedValuesForma2 = <?= json_encode($search_input_forma2); ?>; // PHP'den gelen array

        // Eğer selectedValues varsa, sonuçlara ekle
        if (selectedValues && selectedValues.length > 0) {
            console.log(selectedValues);
            selectedValues.forEach(function (item) {
                $('#search_input_nakliye').append(new Option(item.code,item.nakliye_id, true, true));
            });
        }

        if (selectedValuesForma2 && selectedValuesForma2.length > 0) {
            console.log(selectedValuesForma2);

            selectedValuesForma2.forEach(function (item) {
                $('#forma_2_id').append(new Option(item.invoice_no,item.id, true, true));
            });
        }
        ...
        $('#search_input_nakliye').select2({
            ...
            ajax: {
                url: "<?php echo base_url('qaime/search_cari_to_nakliye'); ?>",
                dataType: 'json',
                delay: 250,
                data: function (params) {
                    return {
                        search: params.term,
                        cari_id: $('#customer_id').val()
                    };
                },
                processResults: function (data) {
                    if (data.status === 200) {
        ...
        $('#forma_2_id').select2({
            placeholder: "Forma 2 aramak için yazın...",
            allowClear: true,
            language: {
                inputTooShort: function () {
                    return "3 karakter daha yazın...";
            ...
            ajax: {
                url: "<?php echo base_url('qaime/search_cari_to_forma_2'); ?>",
                dataType: 'json',
                delay: 250,
                data: function (params) {
                    return {
    ...
    $('#create_form').on('click',function (){
        ...
        if (validateForm() && validateProductTable()) {
            $.confirm({
                ...
                smoothContent: true,
                draggable: false,
                content: 'Qaime Düzenlenecek Emin Misiniz?',
                buttons: {
                    formSubmit: {
                        text: 'Evet',
                        btnClass: 'btn-blue',
                        action: function () {
                            ...
                            $.post(baseurl + 'qaime/update_save', {data: JSON.stringify(data)}, (response) => {
