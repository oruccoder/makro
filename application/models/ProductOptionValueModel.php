...
class ProductOptionValueModel extends CI_Model
{

    var $column_order = array('product_option_value.name', 'product_option_value.description', 'product_option_value.product_option_id');
    var $column_search = array('product_option_value.name', 'product_option_value.description', 'product_option_value.product_option_id');
    ...
    {
        $arr = $this->_get_datatables_query_details_list();
    ...
    private function _get_datatables_query_details_list()
    {
        ...
        $this->db->select('product_option_value.* , product_options.name as product_option_name');
        $this->db->from('product_option_value');
        $this->db->where('product_option_value.deleted_at', NULL, FALSE);
        $this->db->join('product_options', 'product_option_value.product_option_id=product_options.id');
    ...
    public function update()
    {
        ...
        if ($this->db->where(['id' => $id])->update('product_option_value', $data)) {
            return [
                'status' => 1,
            ];
        } else {
    ...
    public function delete()
    {
        ...
        if ($this->db->where(['id' => $id])->update('product_option_value', ['deleted_at' => date('Y-m-d H:i:s')])) {
            return [
                'status' => 1,
            ];
        } else {
    ...
    public function details_item($id)
    {
        $this->db->select('*');
        $this->db->from('product_option_value');
        $this->db->where('product_option_id', $id);
        $query = $this->db->get();
        return $query->result();
    }
