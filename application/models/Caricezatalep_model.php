...
class Caricezatalep_model extends CI_Model
{
...

    {
        ...
        $this->db->where('talep_form_customer_new.type',3);
    ...
    public function create_save(){
        ...
        if ($this->db->insert('talep_form_customer_new', $data)) {
            ...
            $this->aauth->applog("Cari Ceza Talebi Oluşturuldu  : Talep No : ".$talep_no, $this->aauth->get_user()->username);
    ...
    public function form_total_bekleyen(){
        ...
        $this->db->where_not_in('talep_form_customer_new.status',[10,9]);
        $query = $this->db->get();
        return $query->row()->total;
    }
 ...
 <address>' . $this->config->item('address') . '<br>' . $this->config->item('address2') . '</address>
    ...
    public function ajaxlist_bekleyen()

    {
        $this->_ajaxlist_bekleyen();
...
//        echo $this->db->last_query();
    ...
    private function _ajaxlist_bekleyen()

    {
        ...
        $this->db->select('talep_form_customer_new.*,geopos_employees.name as pers_name,progress_status.name as progress_name,"caricezatalep" as href,
    ...
    public function count_filtered_cariceza()
    {
        $this->_ajaxlist_bekleyen();
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all_cariceza()
    {
        $this->_ajaxlist_bekleyen();
        return $this->db->count_all_results();
    }
    ...
    public function caricezatalepcount(){
        $where_talep_form='';
        if($this->session->userdata('set_firma_id')){
            $where_talep_form.='and talep_form_customer_new.loc='.$this->session->userdata('set_firma_id');
            //$this->db->where('virman.loc =', $this->session->userdata('set_firma_id')); //2019-11-23 14:28:57
        }
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `talep_onay_customer_new` 
INNER JOIN talep_form_customer_new On talep_onay_customer_new.talep_id=talep_form_customer_new.id
WHERE talep_onay_customer_new.type = 1 AND
        talep_form_customer_new.status=1 and
        talep_form_customer_new.type=3 and
      talep_onay_customer_new.user_id = $aauth_id AND `staff` = 1 $where_talep_form
")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
