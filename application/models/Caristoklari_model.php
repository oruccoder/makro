...
class Caristoklari_model extends CI_Model
{
...

    {
        ...
        $this->db->where('stock_io.fis_tur',3);
        $i = 0;
            ...
            if($this->db->insert('stock_io', $data_items)){
                ...
                if($index){
                    if ($this->db->insert_batch('stock_io_products', $product_list)) {
                        ...
                        $this->aauth->applog("Cari Stok ÇIKIŞ / Giriş Fişi Oluşturuldu: Talep ID : ".$stock_io_id, $this->aauth->get_user()->username);
        ...
        if($user_id==$details->aauth_id){
            ...
            if(isset($post_data)){
                ...
                if($this->db->update('stock_io', $data_items)){
                    ...
                    if($index){
                        if ($this->db->insert_batch('stock_io_products', $product_list)) {
                            $this->aauth->applog("Cari Stok ÇIKIŞ / Giriş Fişi güncellendi: Talep ID : ".$stock_io_id, $this->aauth->get_user()->username);
                            return [
                                'status'=>1,
    ...
    public function delete_fis(){
        ...
        if($user_id==$details->aauth_id){
            ...
            if( $this->db->delete('stock_io', array('id' => $stock_io_id))){
                $this->aauth->applog("Cari Stok ÇIKIŞ / Giriş Fişi Silindi: Talep Code : ".$code, $this->aauth->get_user()->username);
                return [
                    'status'=>1,
