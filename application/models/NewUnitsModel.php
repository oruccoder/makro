...
class NewUnitsModel extends CI_Model

{
    var $table_news = 'stock_io ';

    var $column_order = array(null, 'geopos_units.name', 'geopos_units.code');

    var $column_search = array('geopos_units.name', 'geopos_units.code');

    var $urun = array('geopos_units.id', 'DESC');
    ...
    private function _get_datatables_query_details_list()
    {
        ...
        $this->db->where('geopos_units.loc =', $this->session->userdata('set_firma_id')); //2019-11-23 14:28:57
        $i = 0;
    ...
    public function update()
    {
        ...
        if ($this->db->where(['id' => $unit_id])->update('geopos_units', $units)) {
            return [
                'status' => 1,
            ];
        } else {
    ...
    public function delete()
    {
        ...
        if ($this->db->where(['id' => $unit_id])->delete('geopos_units')) {
            return [
                'status' => 1,
