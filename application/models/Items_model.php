...

{
    ...
    var $column_order = array(null, 'items.id','items.name', 'items.code','geopos_hrm.val1','geopos_employees.name');

    var $column_search = array('items.name', 'items.code','geopos_employees.name','geopos_hrm.val1');

    var $urun = array('items.id', 'DESC');
    ...
    {
        $this->db->select('items.*,geopos_employees.name as personel_name,geopos_hrm.val1 as departman_name');
        $this->db->from('items');
        $this->db->join('geopos_employees','items.personel_id=geopos_employees.id','left');
        $this->db->join('geopos_hrm','items.departman_id=geopos_hrm.id','left');
        $this->db->where('items.loc =', $this->session->userdata('set_firma_id')); //2019-11-23 14:28:57
        $i = 0;
    ...
    public function update()
    {
        ...
        if ($this->db->where(['id' => $unit_id])->update('items', $units)) {
            return [
                'status' => 1,
            ];
        } else {
    ...
    public function delete()
    {
        ...
        if ($this->db->where(['id' => $unit_id])->delete('items')) {
            return [
                'status' => 1,
