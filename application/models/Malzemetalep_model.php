...
class Malzemetalep_model extends CI_Model
{
...
WHERE talep_onay_new.type = 3 AND  talep_form.status=17 and talep_form.talep_type=1 and talep_onay_new.user_id = $aauth_id AND `staff` = 1 $where_talep_form
")->num_rows();
...
//                                  GROUP BY siparis_list_form.talep_id")->num_rows();
        ...
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `talep_user_satinalma` INNER JOIN  talep_form ON talep_user_satinalma.talep_id=talep_form.id WHERE talep_form.talep_type=1 and talep_user_satinalma.status = 1 AND  talep_form.status IN(2,3)  AND talep_user_satinalma.user_id = $aauth_id")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
        ];
    ...
    public function ihalelistcount(){
        ...
        $count = $this->db->query("SELECT * FROM `talep_onay_new` INNER JOIN talep_form On talep_onay_new.talep_id=talep_form.id WHERE 
                                        ...
                                        AND talep_form.status=4 and talep_form.talep_type=1 and talep_onay_new.staff = 1 $where_talep_form")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
        ];
    ...
    talep_form.id = talep_user_satinalma.talep_id WHERE talep_form.status = 5 AND talep_form.talep_type = 1 AND
    ...
    public function siparis_finist_list_count(){
        ...
        $count = $this->db->query("SELECT * FROM `siparis_list_form`
         INNER JOIN talep_form On siparis_list_form.talep_id=talep_form.id
         WHERE  siparis_list_form.staff_id = $aauth_id AND siparis_list_form.staf_status = 1 and talep_form.status=5 and talep_form.talep_type=1  and siparis_list_form.deleted_at is NULL  
    ...
    ON talep_form.id = talep_user_satinalma.talep_id WHERE talep_form.status = 6 AND talep_form.talep_type=1 and
                                       talep_user_satinalma.user_id = $aauth_id  $where_talep_form")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
    ...
    public function tehvil_list_count(){
        ...
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `talep_form` INNER JOIN geopos_warehouse
    On talep_form.warehouse_id =geopos_warehouse.id  WHERE talep_form.status = 7 and talep_form.talep_type = 1 and
...
geopos_projects On talep_form.proje_id =geopos_projects.id  WHERE talep_form.status = 8 and talep_form.talep_type =1  $where_talep_form")->num_rows();
    ...
    public function warehousetransfercount(){
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `stock_transfer_item_notification` Where staff_id=$aauth_id and staff_status=1 and status=0")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
        ];
    ...
    public function talepwarehousetransfercount(){
        ...
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `warehouse_teslimat_transfer` Where user_id=$aauth_id and status=0")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
        ];
    ...
    public function ihalebeklyenlistcount(){
        ...
        $aauth_id = $this->aauth->get_user()->id;
        $count = $this->db->query("SELECT * FROM `talep_form`  INNER JOIN talep_user_satinalma ON
    talep_form.id = talep_user_satinalma.talep_id WHERE talep_form.status = 3 AND talep_form.talep_type=1 and 
                                                        talep_user_satinalma.user_id = $aauth_id  $where_talep_form")->num_rows();
        return [
            'status'=>1,
            'count'=>$count
