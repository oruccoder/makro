...
class Printerler extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('printerler_model', 'model');
        $this->load->library("Aauth");
    ...
    public function index()
    {
        ...
        if (!$this->aauth->premission(88)->read) {
            ...
            $head['title'] = "Printer / Bilgisayar Tanımlamaları";

            $this->load->view('fixed/header', $head);

            $this->load->view('printerler/index');
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $item) {
            ...
            $view = "<a class='btn btn-info btn-sm ' href='/demirbas/item_views/$item->id/printerler' ><i class='far fa-eye'></i></a>&nbsp;";
    ...
    public function barcode_creates(){
        ...
        if($str)
        {
            $this->session->set_userdata('printers_id',$str);
            echo json_encode(array('code' => 200, 'message'=>'Başarıyla Barkod Oluşturuldu Sayfaya Girmek İçin Tamam Butonuna Tıklayın','url' => '/printerler/barcode_print'));
    ...
    public function barcode_print(){
        $str  = $this->session->userdata('printers_id');
        $data['details'] =$this->db->query("Select * From printerler Where id IN($str)")->result();
