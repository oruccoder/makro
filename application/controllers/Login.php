...
class Login extends CI_Controller
{
    ...
    public function checklogin()
    {
        ...
        $firma_kodu = $this->input->post('firma_kodu');
        $remember_me = $this->input->post('remember_me');
        $rem = false;
        if ($remember_me == 'on') {
            $rem = true;
        }
        if ($this->aauth->Firmalogin($firma_kodu, $rem, $this->captcha)) {
            ...
            redirect('/login/?e=eyxde', 'refresh');
        }
    ...
    public function reset_change()
    {
        ...
        if (strlen($password) > 5) {
            ...
            else echo json_encode(array('status' => 'Error', 'message' => "Code Expired! <a href='" . base_url() . "' class='btn btn-indigo btn-md'><span class='icon-home' aria-hidden='true'></span> " . $this->lang->line('Login') . "  </a>"));
        }
