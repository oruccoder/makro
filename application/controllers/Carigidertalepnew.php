...
class Carigidertalepnew Extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('Carigidertalepnew_model', 'model');
    ...
    public function index()
    {
        ...
        $head['title'] = 'Cari Gider Talep Formu';
        ...
        $this->load->view('carigidertalepnew/index',$data);
    ...
    public function index_old()
    {
        ...
        $head['usernm'] = $this->aauth->get_user()->username;
        $head['title'] = 'Cari Gider Talep Listesi';
        $data['totals']=amountFormat($this->model->form_total_bekleyen());
        $data['bank']=amountFormat($this->model->form_total_bekleyen_method(3));
        $data['nakit']=amountFormat($this->model->form_total_bekleyen_method(1));
        $this->load->view('fixed/header', $head);
        $this->load->view('carigidertalepnew/index_old',$data);
        $this->load->view('fixed/footer');
    }

    public function talep_sepeti()
    {
        ...
        $data['nakit']=amountFormat($this->model->form_total_bekleyen_method(1));
        $this->load->view('fixed/header', $head);
        $this->load->view('carigidertalepnew/talep_sepeti',$data);
        $this->load->view('fixed/footer');
    }
...
//            $row[] = $invoice_durumu;
    ...
    public function create_save(){
        ...
        if (!$this->aauth->premission(92)->write) {
            ...
            if($result['status']){
                ...
                echo json_encode(array('status' => 200, 'message' =>"Başarılı Bir Şekilde Talep Oluşturuldu",'index'=>'/carigidertalepnew/view/'.$result['id']));
    ...
    public function create_save_cart(){
        ...
        if($result['status']){
            echo json_encode(array('status' => 200, 'message' =>"Başarılı bir Şekilde Eklendi",'index'=>'/carigidertalepnew/view/'.$result['id']));
            $this->db->trans_complete();
        }
        else {
            $this->db->trans_rollback();
    ...
    public function view($id)
    {
        ...
        if($data['details']->demirbas_id==-1){
            $this->load->view('carigidertalepnew/view_sepet',$data);
        }
        else {
            $this->load->view('carigidertalepnew/view',$data);
        }
    ...
    public function delete_item_form(){
        ...
        if($yetkili_kontrol || $user_id==$details_form->aauth || $user_id==$details_form->talep_eden_user_id) {
            if($type==1){
