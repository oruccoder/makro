    ...
    public function onayla_action()
{
    ...
    if (!$customer_id) {
        echo json_encode(['success' => false, 'message' => 'Müştəri ID-si göndərilməyib!']);
        return;
    }

    $this->load->model('Customers_model');

    $result = $this->Customers_model->approve_customer($customer_id);

    if ($result) {
        echo json_encode(['success' => true, 'message' => 'Müştəri uğurla təsdiqləndi!']);
    } else {
        echo json_encode(['success' => false, 'message' => 'Müştəri təsdiqləmə zamanı xəta baş verdi!']);
    }
}
    ...
    public function file_handling()
    {
        if ($this->input->get('op')) {
            ...
            $this->load->library("Uploadhandler_generic", array(
                ...
                'accept_file_types' => '/\.(pdf|gif|jpe?g|png|xlsx)$/i',
                'upload_dir' => FCPATH . 'userfiles/documents/',
                'upload_url' => base_url() . 'userfiles/documents/'
                    ...
                    if ($invoices['type_id'] == 29 || $invoices['type_id'] == 30) {
                        ...
                        if ($forma2_durum == 1 || $forma2_durum == '') {
                            if ($forma_2_kontorl) {
                                ...
                                $parcali_kontrol = $this->db->query("SELECT * FROM transaction_pay Where invoice_id = $inv_id_  ");
                                if ($parcali_kontrol->num_rows()) {



                                    $parca_total = 0;
                                    $kalan = 0;
                                    foreach ($parcali_kontrol->result() as $items_parcali) {
                                        $parca_total += $items_parcali->amount;
                                    }
                                    $kalan = floatval($borc) - floatval($parca_total);
                    ...
                    if (intval($items->forma2_id == 0)) {
                        ...
                        if ($invoices_kontrol->num_rows()) {
                            $data[] = [
                                ...
                                'end_date_islem' => $invoices_kontrol->row()->end_date_islem,
        ...
        if ($data) {
            foreach ($data as $value) {
                $data_parcalis = array(
                    ...
                    'notes' => 'Avans Parçalı',
                    'eid' => $value['aauth_id'],
                    'invoicedate' => $value['end_date_islem'],
                    'csd' => $value['cari_id'],
                    'loc' => 5,
                    'invoice_type_id' => 76,
                ...
                if ($this->db->insert('geopos_invoices', $data_parcalis)) {
                    ...
                    $this->db->where('id', $value['transaction_pay_id']);
