function selectCustomer(cid, cname, cadd1, cadd2, ph, email,discount=0,kalan_kredi,kredi,invoice_type,company,masraf_id) {
...
$(document).ready(function () {
    ...
    $("#customer-box").keyup(function () {
        var invoice_type=$("#invoice_type :selected").val();
        $.ajax({
            type: "GET",
            url: baseurl + 'search_products/csearch',
            data: 'keyword=' + $(this).val()+'&invoice_type=' + invoice_type +'&'+crsf_token+'='+crsf_hash,
            beforeSend: function () {
    ...
    $('#warehouses').change(function () {
        ...
        $.ajax({
            ...
            data: 'invoice_type=' + invoice_type + '&wid=' + whr + '&cid=' + cat + '&' + crsf_token + '=' + crsf_hash,
            success: function (data) {
...
function payInvoice(pyurl) {
    ...
    if (errorNum > 0) {
        ...
        $("#notify .message").html("<strong>Hata</strong>: It appears you have forgotten to enter partial amount!");
...
function loadSmsTem (action) {
  ...
  $('#submitSMS').on('click', function (e) {
    ...
    if (errorNum > 0) {
        ...
        $("#notify .message").html("<strong>Hata</strong>: It appears you have forgotten to enter mobile number!");
        $("html, body").animate({scrollTop: $('#notify').offset().bottom}, 1000);
    } else {
        jQuery.ajax({
...
function sendBill(message) {
    ...
    if (errorNum > 0) {
        $("#notify").removeClass("alert-success").addClass("alert-warning").fadeIn();
        $("#notify .message").html("<strong>Hata</strong>: It appears you have forgotten to enter email!");
        $("html, body").animate({scrollTop: $('#notify').offset().bottom}, 1000);
    } else {
        jQuery.ajax({
            url: baseurl + 'communication/send_invoice_new',
            type: 'POST',
...
$(document).on('click', ".delete-object", function (e) {
    ...
    document.cookie = "invoice_id="+$('#object-id').val();
...
function actionProduct(actionurl) {
    ...
    if (errorNum > 0) {
        $.alert({
            ...
            columnClass: "small",
            title: 'Dikkat!',
            content: 'Lüten Zorunlu Alanları Doldurunuz!!',
            buttons: {
                prev: {
                    text: 'Tamam',
...
var errorNum = farmCheck();
    if (errorNum > 0) {
        $("#notify").removeClass("alert-success").addClass("alert-warning").fadeIn();
        $("#notify .message").html("<strong>Hata</strong>: It appears you have forgotten to details!");
